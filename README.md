...
- **명령 지시문**:
```text
1. 최근 24시간 글로벌 IT/AI 뉴스를 조사하되 숏폼(숏츠, 릴스 등) 관련 가벼운 토픽은 제외해. 중요도 및 발표량에 맞춰 최소 6개에서 최대 10개의 핵심 주제를 선정하고(6개 고정 금지), 각 주제별로 2개 이상의 교차 기사를 검색해 related_urls에 담아 news_data.json을 작성해. (대형 이벤트 여부에 따라 daily/event 모드 자동 판별)
